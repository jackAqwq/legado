    ...
    fun build_entries_should_output_all_named_files() {
        val entries = PerformanceMetricsBatchExportBuilder.buildEntries(
            ...
            rssFailureSlowestLines = listOf(
                "4|rss.intercept|90ms|source=BottomWebViewDialog,result=failure"
            ),
            generatedAtMs = 1234L
        )
        ...
        assertEquals("performance_metrics_rss_failure_slowest_20.txt", entries[5].fileName)
        ...
        assertTrue(entries[5].text.contains("summary.p95_duration_ms=90"))
    }
